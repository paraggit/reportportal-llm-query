...
class DataNormalizer:
    ...
    def normalize_test_executions(
        ...
        for test in test_executions:
            data.append({
                ...
                'end_time': datetime.fromtimestamp(test.endTime / 1000) if test.endTime else None,
                'duration_seconds': (test.endTime - test.startTime) / 1000 if test.endTime else None,
    ...
    def create_test_summary(df: pd.DataFrame) -> Dict[str, Any]:
        ...
        flaky_tests = test_status_counts[
            (test_status_counts.get('PASSED', 0) > 0) & 
            (test_status_counts.get('FAILED', 0) > 0)
        ].index.tolist()
